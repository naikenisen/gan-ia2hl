...
# Dictionnaires globaux pour stocker les métriques de registration
registration_metrics = defaultdict(lambda: defaultdict(dict))
global_registration_metrics = {}  # Pour stocker les métriques de registration globale par patient
...
def compute_global_registration(img_fixed, img_moving, patient_id):
    """
    Calcule la transformation affine globale entre deux lames complètes (en basse résolution).
    Retourne la transformation et les métriques de qualité.
    """
    print("\n[REGISTRATION GLOBALE] Calcul de la transformation sur la lame entière...")
    ...
    # Détection ORB avec beaucoup de features pour la lame entière
    ...
    print(f"  Keypoints détectés - HES: {len(kp1) if kp1 else 0}, CD30: {len(kp2) if kp2 else 0}")
    ...
    num_good_matches = min(len(matches), max(100, int(len(matches) * 0.30)))
    good_matches = matches[:num_good_matches]
    
    print(f"  Matches trouvés: {len(matches)}, Bons matches: {len(good_matches)}")
    ...
    try:
        ...
        if model is None or np.sum(inliers) < 10:
        ...
        print(f"  {symbol} {num_inliers} inliers sur {len(good_matches)} matches")
        print(f"  Ratio: {inlier_ratio:.1%} - Qualité: {quality}")
        
        # Créer une visualisation de la registration globale
        fig, axes = plt.subplots(1, 3, figsize=(24, 8))
        
        # Image HES
        axes[0].imshow(img_fixed)
        axes[0].set_title(f'HES (référence)\n{img_fixed.shape[1]}x{img_fixed.shape[0]} pixels', 
                         fontsize=14, fontweight='bold')
        axes[0].axis('off')
        
        # Image CD30 originale
        axes[1].imshow(img_moving)
        axes[1].set_title(f'CD30 (avant alignement)\n{img_moving.shape[1]}x{img_moving.shape[0]} pixels', 
                         fontsize=14, fontweight='bold')
        axes[1].axis('off')
        
        # Appliquer la transformation globale
        h, w = img_fixed.shape[:2]
        aligned_global = cv2.warpAffine(
            img_moving, 
            model.params[:2], 
            (w, h),
        ...
        # Image CD30 alignée
        axes[2].imshow(aligned_global)
        axes[2].set_title(f'CD30 (après alignement global)\n{num_inliers} inliers ({inlier_ratio:.1%}) - {quality}', 
                         fontsize=14, fontweight='bold', 
                         color='green' if inlier_ratio >= 0.6 else 'orange')
        axes[2].axis('off')
        ...
        # Logger dans wandb
        wandb.log({
            f"{patient_id}/global/keypoints_hes": len(kp1),
            f"{patient_id}/global/keypoints_cd30": len(kp2),
            ...
            f"{patient_id}/global/quality": quality,
        ...
        global_registration_metrics[patient_id] = metrics
        ...
        # Stocker l'échec
        global_registration_metrics[patient_id] = {
            'inliers': 0,
            'total_matches': 0,
            'ratio': 0.0,
            'quality': 'FAILED',
...
def visualize_orb_detection_and_matching(img_fixed, img_moving, patient_id, region_idx, global_transform=None):
    """
    Visualise la détection ORB et l'appariement des points.
    Si global_transform est fourni, l'applique d'abord avant la registration locale.
    Retourne aussi la transformation calculée.
    """
    # Appliquer la transformation globale si disponible
    if global_transform is not None:
        h, w = img_fixed.shape[:2]
        img_moving = cv2.warpAffine(
            img_moving,
            global_transform.params[:2],
            (w, h),
            flags=cv2.INTER_LINEAR,
            borderMode=cv2.BORDER_CONSTANT,
            borderValue=(255, 255, 255)
...
def generate_registration_csv(output_folder):
    ...
    Inclut également les métriques de registration globale.
    """
    ...
    for patient_id in sorted(registration_metrics.keys()):
        ...
        # Ajouter la métrique de registration globale
        if patient_id in global_registration_metrics:
            global_metrics = global_registration_metrics[patient_id]
            if global_metrics['status'] == 'success':
                row['Global_Registration'] = f"{global_metrics['ratio']:.3f}"
            else:
                row['Global_Registration'] = f"0.000 ({global_metrics['status']})"
        else:
            row['Global_Registration'] = "N/A"
    ...
    # Réorganiser les colonnes pour avoir Patient_Type, Global_Registration en premier, puis les régions triées
    region_cols = sorted([col for col in df.columns if col.startswith('Region_')], 
                        key=lambda x: int(x.split('_')[1]))
    df = df[['Patient_Type', 'Global_Registration'] + region_cols]
    ...
    # Ajouter d'abord les métriques de registration globale
    for patient_id in sorted(global_registration_metrics.keys()):
        global_metrics = global_registration_metrics[patient_id]
        detailed_rows.append({
            'Patient': patient_id,
            'Type': 'HES/CD30',
            'Region': 'GLOBAL',
            'Inliers': global_metrics['inliers'],
            'Total_Matches': global_metrics['total_matches'],
            'Ratio': global_metrics['ratio'],
            'Quality': global_metrics.get('quality', 'N/A'),
            'Status': global_metrics['status']
        })
    
    # Puis ajouter les métriques locales par région
    for patient_id in sorted(registration_metrics.keys()):
        patient_data = registration_metrics[patient_id]
...
def process_one_slide_pair_visualization(hes_path, cd30_path):
    ...
    # PHASE 1: Registration globale sur la lame entière
    print("\n[2/4] Registration globale sur la lame entière...")
    global_transform, global_metrics = compute_global_registration(
        lowres_hes_np, 
        lowres_cd30_np, 
        patient_id
    )
    ...
    if global_transform is None:
        print("⚠ Attention: La registration globale a échoué, les régions seront traitées sans pré-alignement")
    else:
        print(f"✓ Registration globale réussie: {global_metrics['inliers']} inliers, ratio={global_metrics['ratio']:.1%}")
    
    # Calculer le masque de tissu
    print("\n[3/4] Calcul du masque de tissu...")
    mask_hes = compute_tissue_mask(lowres_hes)
    
    # PHASE 2: Registration locale sur les sous-régions
    ...
    for region_y in range(0, h0_hes, stride):
        for region_x in range(0, w0_hes, stride):
            ...
            if global_transform is not None:
                print(f"    (avec transformation globale pré-appliquée)")
            transformation, aligned_img, inliers, success = visualize_orb_detection_and_matching(
                region_hes_lr, region_cd30_lr, patient_id, region_idx, global_transform
            )
    ...
    # Ajouter les métriques de la registration globale au résumé
    if global_metrics is not None:
        summary_log[f"{patient_id}/global_registration_success"] = True
        summary_log[f"{patient_id}/global_inlier_ratio"] = global_metrics['ratio']
    else:
        summary_log[f"{patient_id}/global_registration_success"] = False
